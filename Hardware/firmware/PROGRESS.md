...
- 🔵 **Carrier = 104.1667 kHz, FIXED for every board and every user** (sysclk 150 MHz,
  ...
  - **Margin, computed 2026-08-28:** a square-wave demodulator responds at odd harmonics, so
    interference folds to |f_i − n·f_c|. Against the LM5157's nominal 1.055 MHz the nearest
    odd harmonics are **9·f_c = 937.5 kHz (117.5 kHz away)** and **11·f_c = 1145.8 kHz
    (90.8 kHz away)**, against a 15.39 kHz passband. **The boost can drift −9.6 % / +7.1 %
    before anything folds in.**
...
>
> 🔴 **FIRST: run `hpf test` — it has NEVER run on board 3.** `hpf` says so. §3.4 and
> §3.5 were both completed with the polarity unverified. The §3.5 high point proves it
> incidentally (HOLD held a 2225-code step for seconds), but nothing has RECORDED it,
> and `hpf test` also gives the switch-leakage number. 32 s, then `cfg save`.
>
> **Then, in order:**
> 1. **Fix the scope ground** — the 3.6b capture carried 12 mV of common-mode 60 Hz
>    (ratio 1.00 on both channels). It puts a floor under every small number.
> 2. **§3.6b Method A** — beam OFF, `hpf hold`, then step the PSU 5.20 → 5.10 V by hand.
>    Zero optical change, so whatever moves is Q8. Compare against ×7.25.
> 3. **`detect`** — 30 s, confirms GPIO46 reads and the SM arms. Never run on board 3.
> 4. **§3.6 Check 1** — record the LM5157 SW frequency. Never done on ANY board.
> 5. **§3.6 Check 2** — `scan carrier 95000 115000 9`, ~5 min. Looking for FLAT, not a
>    winner.
> 6. **§3.7.**
>
> 🔵 **The carrier is FIXED at 104.1667 kHz** (decided 2026-08-28, §8). §3.6 is no longer
> a per-board scan — it is a one-off robustness check.
>
